...
<div id="myModal" class="modal">
  <div class="modal-content">
    ...
    <p style="margin-top: 10px;">Exam Created</p>
  </div>
...
function addq(i)
{
  ...
  var table = document.getElementById("QuestionBank");
    ...
    var test2 = row.insertCell(6);
    var btn = document.createElement("BUTTON");
    btn.setAttribute( "class", "b" );
    btn.setAttribute( "onClick", "remq("+(examSize-1)+")" );
    btn.innerHTML="X";
...
function remq(i)
{
  var table = document.getElementById("ExamQuestions").children[0];
  table.removeChild(table.children[i+1]);
   question_ID_list.splice(i, 1);
    question_content_list.splice(i, 1);
    question_type_list.splice(i, 1);
    question_difficulty_list.splice(i, 1);
    question_constraint_list.splice(i, 1);
    examSize--;
   for(var j = i; j < examSize; j++)
   {
     var input = table.children[j+1].children[5].children[0];
     input.setAttribute( "id", "pointInput"+j );
     var btn = table.children[j+1].children[6].children[0];
     btn.setAttribute( "onClick", "remq("+(j)+")" );
   }
